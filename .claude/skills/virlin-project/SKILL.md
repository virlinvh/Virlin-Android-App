---
name: virlin-project
description: Virlin product, domain and frozen-UI rules for the Virlin Android app (human-attention orchestration system). Load for product behaviour, WorkStream states, what is frozen or approved, current development status, and tool/skill selection. For animation and Orb interaction load virlin-motion-interaction; for writing Compose code load virlin-android-compose; for verification and diagnosis load virlin-mobile-qa; for domain models, actions and persistence load virlin-data-domain.
---
